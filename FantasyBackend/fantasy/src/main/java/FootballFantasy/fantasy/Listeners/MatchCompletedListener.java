...
public class MatchCompletedListener {
    ...
    public void handleMatchCompletedAfterCommit(MatchCompletedEvent event) {
        System.out.println("🎯 AFTER_COMMIT EVENT LISTENER TRIGGERED: Match ID " + event.getMatchId() + " completed");
        ...
        try {
            ...
            System.err.println("❌ Error in after-commit match completion handler: " + e.getMessage());
            e.printStackTrace();
        }

        System.out.println("🎯 AFTER_COMMIT EVENT LISTENER COMPLETED for match ID " + event.getMatchId());
    }
